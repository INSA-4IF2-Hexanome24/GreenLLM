...
class GreenKNNRouter(MetaRouter):
    ...
    def __init__(self, yaml_path: str):
        ...
        self.enable_timing = hparam.get("enable_timing", True)
        self._log_timestamp("init: MetaRouter loaded", init_start)
        ...
        self._log_timestamp("init: KNN classifier initialized", init_start)
        ...
        self._log_timestamp("init: difficulty estimator initialized", init_start)
        ...
        self._log_timestamp("init: CO2 data loaded", init_start)

        # Poids de la fonction d'utilité
        self.w_perf = hparam.get("w_perf", 1.0)
        self.w_co2  = hparam.get("w_co2",  0.3)
        
        # Poids du coefficient de difficulté
        # (contrôle l'intensité du bonus/malus basé sur la difficulté)
       ...
       # ------------------------------------------------------------------
        ...
        self._log_timestamp("init: KNN training data prepared", init_start)
        ...
        if diff_path:
            ...
            self._log_timestamp("init: difficulty model load check completed", init_start)
        ...
        self._log_timestamp("init: GreenKNNRouter fully initialized", init_start)

    def _log_timestamp(self, step_name: str, step_start: Optional[float] = None) -> None:
        """Log horodaté avec durée écoulée depuis un point de départ optionnel."""
        if not getattr(self, "enable_timing", True):
            return
        ...
        print(f"[TIMING {now}] {step_name} | elapsed={elapsed:.3f}s")
    ...
    def _compute_utility(
        ...
        Formule (cf. tableau) :
            utility(m) = (w_perf · perf(m) - w_co2 · co2_impact(m)) · coeff(m, difficulty)

        Le coefficient de difficulté favorise les petits modèles quand
        la requête est facile, et les grands modèles quand elle est difficile :
          - Si m in small_models: coeff = 1 + (1 - difficulty) · difficulty_weight
          - Sinon:                 coeff = 1 + difficulty · difficulty_weight
        ...
        Args:
            ...
            difficulty       : score de difficulté ∈ [0, 1]
        ...
        for m in candidate_models:
            ...
            base_utility = self.w_perf * perf_scores[m] - self.w_co2 * co2_norm
            
            # Appliquer le coefficient de difficulté
            if m in self.small_models:
                # Petits modèles : bonus quand la requête est facile
                difficulty_coefficient = 1.0 + (1.0 - difficulty) * self.difficulty_weight
            else:
                # Grands modèles : bonus quand la requête est difficile
                difficulty_coefficient = 1.0 + difficulty * self.difficulty_weight
            
            utility[m] = base_utility * difficulty_coefficient
    ...
    def route_single(self, query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._log_timestamp("route_single: start")
        ...
        self._log_timestamp("route_single: KNN model loaded", step_start)


        # (1) Embedding
        step_start = perf_counter()
        embedding = get_longformer_embedding(query["query"]).numpy()
        self._log_timestamp("route_single: embedding computed", step_start)

        # (2) Difficulté
        step_start = perf_counter()
        difficulty = self._estimate_difficulty(embedding)
        self._log_timestamp("route_single: difficulty estimated", step_start)

        # (3) Sélection des candidats
        step_start = perf_counter()
        all_models = self._get_candidate_models()
        candidates = all_models  # Tous les modèles sont candidats
        self._log_timestamp("route_single: candidates selected", step_start)

        # (4) Score de performance KNN
        step_start = perf_counter()
        perf_scores = self._knn_perf_scores(embedding, candidates)
        self._log_timestamp("route_single: KNN perf scores computed", step_start)

        # (5) Score d'utilité (avec coefficient de difficulté)
        step_start = perf_counter()
        utility_scores = self._compute_utility(perf_scores, candidates, difficulty)
        self._log_timestamp("route_single: utility scores computed", step_start)

        # (6) Sélection du meilleur modèle
        step_start = perf_counter()
        best_model = max(utility_scores, key=utility_scores.__getitem__)
        self._log_timestamp("route_single: best model selected", step_start)
        ...
        self._log_timestamp("route_single: completed", total_start)
        return output
    ...
    def route_batch(
        ...
        self._log_timestamp("route_batch: start")
        ...
        self.knn_model = load_model(load_knn_path)
        self._log_timestamp("route_batch: KNN model loaded", step_start)
        ...
        if diff_path:
            ...
            self._log_timestamp("route_batch: difficulty model load check completed", step_start)
        ...
        for i, row in enumerate(query_data):
            ...
            self._log_timestamp(f"route_batch[{i}]: start")
            ...
            self._log_timestamp(f"route_batch[{i}]: embedding computed", step_start)

            step_start = perf_counter()
            difficulty = self._estimate_difficulty(embedding)
            all_models = self._get_candidate_models()
            candidates = all_models  # Tous les modèles sont candidats
            self._log_timestamp(f"route_batch[{i}]: difficulty estimated and candidates selected", step_start)

            step_start = perf_counter()
            perf_scores    = self._knn_perf_scores(embedding, candidates)
            utility_scores = self._compute_utility(perf_scores, candidates, difficulty)
            best_model     = max(utility_scores, key=utility_scores.__getitem__)
            self._log_timestamp(f"route_batch[{i}]: KNN+utility computed and best model selected", step_start)
            ...
            if row_task_name:
                ...
                self._log_timestamp(f"route_batch[{i}]: prompt formatting completed", step_start)
            else:
                query_text_for_execution = original_query
            ...
            try:
                ...
                self._log_timestamp(f"route_batch[{i}]: API call completed", step_start)
            ...
            if ground_truth:
                ...
                self._log_timestamp(f"route_batch[{i}]: task performance computed", step_start)

            results.append(row_copy)
            self._log_timestamp(f"route_batch[{i}]: completed", row_start)

        self._log_timestamp("route_batch: completed", batch_start)
        return results